...
public class Before_Session_Destroyed implements ApplicationListener<SessionDestroyedEvent> {
	 ...
	 * lastLoggedOut in User table. I have configure automatic session
	...
	public void onApplicationEvent(SessionDestroyedEvent event) {
		...
		try {
			...
			log.info("id : " + idUserToSet);
			Date logOutTime = new Date();
			// Block 1 : Update time_Logged_out of Current_User
			Jpa_User_Repository.updateUserTimeLoggedOut(idUserToSet, logOutTime);
			log.info("attribute 'Time_Logged_Out' for user with Email :" + email + " with SessionId :" + SessionId
					+ " has been approved successelly");
			// End Block Update
			// Block 2 : Delete the old session_Id of current_User then adding new one 
			log.info("Old session reference : "+Jpa_User_Repository.getOldSessionId(idUserToSet));
			Jpa_User_Repository.updateUserSessionId(SessionId, idUserToSet);
			log.info("new Session reference has been updated successfully: "+SessionId);
		} catch (Exception exception1) {
			log.error("attribute 'Time_Logged_Out' for user with Email :" + getCurrentUserLogin()
